...
cd $dirNode && npm uninstall node-red-contrib-nibepi && npm install --save anerdins/node-red-contrib-nibepi#smo
echo "Downloading new flows for Node-RED"
mount=$(sudo mount -o remount,rw / 2>/tmp/tar_stderr);
cd /tmp && wget https://raw.githubusercontent.com/anerdins/nibepi-flow/smo/flows.json
